...
|---|---|---|
| Instrument master | Normalized contracts, immutable SQLite snapshots and fixture-tested cache rules | Approved Angel One adapter, published-master validation, local sync command and a real user-owned data check |
...
No broker credentials, OTP/TOTP, account state, live quotes, historical provider call, external scheduler, cloud service or live order is present in the current codebase. The next technical work must remain behind these constraints until separately approved.
